      ...
      <div className="grid grid-cols-1 md:grid-cols-3 mt-5">
        <div className="pt-4 border-r">
          ...
          <div className="mt-5 flex flex-col gap-4">
            ...
            <h2 className="flex gap-2">
              <MapPin />
              {formValue?.locationType?.name} Meeting
            </h2>
            <Link href={formValue?.locationUrl ? formValue.locationUrl : '#'} className="text-primary">
        ...
        <div className="md:col-span-2 flex px-4">
          ...
          <div className="flex flex-col w-full overflow-auto gap-4 p-5" style={{ maxHeight: '400px' }}>
            {timeSlots?.map((time, index) => (
              <Button key={index} className="border-primary text-primary" variant="outline">
                {time}
              </Button>
            ))}
          </div>
